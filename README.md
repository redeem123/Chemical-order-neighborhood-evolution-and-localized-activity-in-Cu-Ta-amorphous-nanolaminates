...
the versioned v1.1.0 supporting dataset is author-held and prepared for a separate deposit. It does not contain
...
v1.1.0 packet, the container, extracted data and generated checks. The supplied manifests, not
rounded manuscript tables, are the numerical inputs.

`reproduce.py` runs nine bounded stages with the matching v1.1.0 dataset. Each stage uses one numerical thread,
...
| `clean-r03` | Verifies both clean same-start R03 receipts and 14 selected frame identities; recomputes the half-strength force-window means. Other structural values remain author-derived and are not independently recalculated by this stage. |
...
On 23 September 2026, the local v1.1.0 candidate passed all nine stages. Its
979 indexed files also round-tripped through the HDF5 container with matching
SHA-256 digests; the extracted clean-R03 stage passed again. These are author-run
checks of the local, not-yet-pushed code update and author-held dataset, not a
claim about the currently published GitHub commit or an independent reanalysis.
...
The v1.1.0 dataset indexes the two clean same-start replacements separately.
They are the reported R03 records but not additional material preparations; the
portable `clean-r03` check does not reproduce all structural calculations.
